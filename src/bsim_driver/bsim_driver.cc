...
class VTADevice {
 public:
  VTADevice() {
    // VTA stage handles
    vta_axi_handle_ = VTAMapRegister(0xA0000000);
  }

  ~VTADevice() {
    // Close VTA stage handle
    VTAUnmapRegister(vta_axi_handle_);
  }

  int Run(vta_phy_addr_t insn_phy_addr,
          uint32_t insn_count,
          uint32_t wait_cycles) {
    VTAWriteMappedReg(vta_axi_handle_, 0x10, insn_count);
    VTAWriteMappedReg(vta_axi_handle_, 0x18, insn_phy_addr);
    VTAWriteMappedReg(vta_axi_handle_, 0x20, 0);
    VTAWriteMappedReg(vta_axi_handle_, 0x28, 0);
    VTAWriteMappedReg(vta_axi_handle_, 0x30, 0);
    VTAWriteMappedReg(vta_axi_handle_, 0x38, 0);
    VTAWriteMappedReg(vta_axi_handle_, 0x40, 0);
    
    // VTA start
    VTAWriteMappedReg(vta_axi_handle_, 0x0, VTA_START);
    
    // Loop until the VTA is done
    unsigned t, flag = 0;
    for (t = 0; t < wait_cycles; ++t) {
      flag = VTAReadMappedReg(vta_axi_handle_, 0x50);
