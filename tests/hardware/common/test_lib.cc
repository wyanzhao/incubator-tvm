...
uint64_t vta_bsim(uint32_t instr_count,
  ...
  // Get VTA handles
  void* axi_base_addr = VTAMapRegister(0x00A0000000);
  ...
  VTAWriteMappedReg(axi_base_addr, 0x10, instr_count);
  if (insns) VTAWriteMappedReg(axi_base_addr, 0x18, insn_phy);
  if (inputs) VTAWriteMappedReg(axi_base_addr, 0x28, input_phy);
  if (weights) VTAWriteMappedReg(axi_base_addr, 0x30, weight_phy);
  if (uops) VTAWriteMappedReg(axi_base_addr, 0x20, uop_phy);
  if (biases) VTAWriteMappedReg(axi_base_addr, 0x38, bias_phy);
  if (outputs) VTAWriteMappedReg(axi_base_addr, 0x40, output_phy);

  // VTA start
  VTAWriteMappedReg(axi_base_addr, 0x0, 0x1);

  int flag = 0, t = 0;
  for (t = 0; t < 10000000; ++t) {
    flag = VTAReadMappedReg(axi_base_addr, 0x50);
    if (flag & VTA_DONE) break;
  }
  ...
  // Unmap VTA register
  VTAUnmapRegister(axi_base_addr);
  return t_fpga;
}

#endif // VTA_TARGET_BSIM_ZCU104
#endif  // NO_SIM
...
/* int alu_test_bsim(int opcode, bool use_imm, int batch, int vector_size, bool uop_compression) {
  ...
  // Prepare the uop buffer
  VTAUop * uop_buf = getMapALUUops(tx_size, uop_compression);
  auto uop_size_ = (uop_compression) ? 1 : tx_size;
  ...
  // Invoke the VTA
  uint64_t t_fpga = vta_bsim(ins_size, (uint8_t *)insn_buf, (uint8_t *)uop_buf,
		  (uint8_t *)NULL, (uint8_t *)NULL, (uint8_t *)bias_buf, (uint8_t *)output_buf);
  // Report on timining
  printf("INFO - Synchronization time: %.3fms\n", static_cast<float>(t_fpga) / 1E6);
  printf("INFO - Throughput: %.3fGOps/s\n", static_cast<float>(vector_size * batch) / t_fpga);
#else
  vta_bsim(ins_size, (ap_uint<8> *)insn_buf, ( ap_uint<8> *)uop_buf,
		  ( ap_uint<8> *)NULL, ( ap_uint<8> *)NULL,
		  ( ap_uint<8> *)bias_buf, ( ap_uint<8> *)output_buf, &done);
#endif
...
#if VTA_DEBUG == 1
        printf("DEBUG - CORRECT %d, %d: expected 0x%d but got 0x%d\n", i, j,
               static_cast<int>(outputs_ref[i][j]),
               static_cast<int>(outputs[i][j]));
#endif
      }
...
int blocked_gemm_test_bsim(int batch, int channels, int block, bool uop_compression,
  ...
  int uop_size_ = (uop_compression) ? block / VTA_BATCH : block / VTA_BATCH * block / VTA_BLOCK_IN * block / VTA_BLOCK_OUT;
  if (virtual_threads > 1) uop_size_ *= 2;

  uint32_t done = 0;
#ifdef NO_SIM
  // Invoke the VTA
  uint64_t t_fpga = vta_bsim(ins_size,
                        (uint8_t *)insn_buf,
                        (uint8_t *)uop_buf,
                        (uint8_t *)input_buf,
                        (uint8_t *)weight_buf,
  ...
  vta_bsim(ins_size,
      ...
      ( ap_uint<8> *) uop_buf,
      ( ap_uint<8> *) input_buf,
      ( ap_uint<8> *) weight_buf,
      ( ap_uint<8> *) bias_buf,
      ( ap_uint<8> *) output_buf, &done);
#endif
...
int alu_test_bsim_v1(int opcode, bool use_imm, int batch, int vector_size, bool uop_compression) {
  ...
  printf("INFO - Throughput: %.3fGOps/s\n", static_cast<float>(vector_size * batch) / t_fpga);
#else
  vta_bsim_v1(ins_size, (ap_uint<128> *)insn_buf, ( ap_uint<8> *)uop_buf,
		  ( ap_uint<8> *)NULL, ( ap_uint<8> *)NULL,
		  ( ap_uint<8> *)bias_buf, ( ap_uint<8> *)output_buf);
#endif
...
#else
  // Invoke the VTA
  vta_bsim_v1(ins_size,
      ( ap_uint<128> *) insn_buf,
      ( ap_uint<8> *) uop_buf,
      ( ap_uint<8> *) input_buf,
      ( ap_uint<8> *) weight_buf,
      ( ap_uint<8> *) bias_buf,
      ( ap_uint<8> *) output_buf);
#endif
